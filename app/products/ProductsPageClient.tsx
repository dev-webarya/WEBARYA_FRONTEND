  ...
  return (
    <main>
          ...
          {Object.entries(portfolio).map(([key, category]) => (
            ...
            >
              <CardHeader className="flex flex-col items-center">
                ...
                <CardTitle className="text-xl font-semibold text-center mt-2">{key}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 text-sm text-gray-700">
                <ul className="space-y-3 text-gray-500">
                  {Array.isArray(category.shortDescription)
                    ? category.shortDescription.map((point: string, idx: number) => (
                        <li key={idx} className="hover:text-blue-500 transition-colors duration-200">
                          {point}
                        </li>
                      ))
                    : <li className="hover:text-blue-500 transition-colors duration-200">{category.shortDescription}</li>}
                </ul>
              </CardContent>
            </Card>
          ))}
      ...
      {modalCategory && detailed && (
        <ProductModal isOpen={!!modalCategory} onClose={() => setModalCategory(null)} category={modalCategory} />
      )}
      <WhatsAppFloat />
    </main>
  );
